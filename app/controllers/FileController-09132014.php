...
class FileController extends BaseController {
            ...
            if (Session::has('upload.files')) {
                ...
                if (is_array($uploadFiles) && !empty($uploadFiles) && $batch_id != null) {
                    ...
                    foreach($uploadFiles as $file) {
                        ...
                        $fileModel->milling_type = $uploadFileMillingTypeArray[$fileCount];
                        $fileModel->shipping_type = $uploadFileDeliveryMethodArray[$fileCount];
    ...
    public function getDownloadSingle($file_id = null) {
        ...
        if ($this->user->hasAccess('admin') || $this->user->id == $file->user_id) {
            ...
            return Response::download(public_path() . '/uploads/' . $upload_path . '/' . $file->filename_random, $fileUser->lab_name . '___' . $file->filename_original);
        }
    ...
    public function getDownloadBatch($batch_id = null) {
        ...
        foreach($files as $file) {
            ...
            if (file_exists($file_path) && is_readable($file_path)) {
                if (!$archive->addFile($file_path, $from . '___' . $file->filename_original)) {
