...
class FileController extends BaseController {
    ...
    public function getHistory() {
        if ($this->user->hasAccess('superuser')) {
            ...
            return View::make('user.file.history2')->with('data', $data)->with('batches', $batches);
        }
    }
    ...
    public function getDownloadSingle($file_id = null) {
        ...
        if ($this->user->hasAccess('admin') || $this->user->id == $file->user_id) {
            ...
            return Response::download(public_path() . '/uploads/' . $upload_path . '/' . $file->filename_random, $fileUser->lab_name . '___' . $file->milling_type . '___' . $file->shipping_type . '___' . $file->filename_original);
        }
    ...
    public function getDownloadBatch($batch_id = null) {
        ...
        foreach($files as $file) {
            ...
            if (file_exists($file_path) && is_readable($file_path)) {
                if (!$archive->addFile($file_path, $from . '___' . $file->milling_type . '___' . $file->shipping_type . '___' . $file->filename_original)) {
